...
# ── [오류 1 수정] os.environ["KEY"] → get() + 명시적 예외처리 ──
...
CLOSING_SUBTITLE = CLOSING_NARRATION  # 자막용 별도 가공 없이 그대로 복사
...
- 시가총액의 낭독 기준: 65,400억 → 육만오천사백억원, 3% → 삼퍼센트, 2.5% → 이점오퍼센트
- 거래소 구분 낭독 형태: 코스피→코스피, 코스닥→코스닥, ETF→이티에프, SK→에스케이, LG→엘지
- 주가의 낭독 형태 관련 종목의 현재가를 바탕으로 낭독
## subtitle 작성 지침 — 짧은 자막 최적화 (필수 작성 항목)
subtitle 작성은 화면에 나오는 자막용으로 작성해야 합니다. 최대한 간결하게 화면에 표시되도록 아래와 같이 작성합니다.
- 시가총액의 낭독 기준: 65,400억 → 6.54조원, 3% → 3%
- 시장 구분 표기 형태: KOSPI, HBM, ETF, SK, LG, KB, NH
- 증감/등락 표기 형태: 매출액 → +, 순이익 → -, 영업이익 → +, 매출하락 → -
- 거래소 구분 낭독 형태: 코스피→코스피, 코스닥→코스닥, ETF→이티에프, SK→에스케이, LG→엘지
- narration과는 다르게 화면에 표시되도록 구어체가 아니라 문어체 단어로 화면에 나오도록 작성합니다.
- 서술어나 문장 형식보다는 narration과는 다른 방식으로 자막을 구성 요약 언급 형태로 작성해야 합니다.
...
각 종목별로 1개인 narration_XXX (TTS용)과 subtitle_XXX (자막용)을 짝으로 작성해야 합니다.
...
{{
  ...
  "sections": [
    ...
    {{
      "id": "market_summary",
      "label": "시장 요약",
      "narration": "오늘의 종합 시장을 알아보겠습니다. [TTS용 — 시가 총액, 등락, 등 낭독]",
      "subtitle": "오늘의 종합 시장을 알아보겠습니다. [자막용 — 시가총액, 등락, 등 낭독]",
      "kospi_value": "2,650",
      "kospi_change": "+1.2%",
    ...
    {{
      ...
      "subtitle": "오늘의 업종 분석을 알아보겠습니다. [자막용]",
      "sector_list": [
    ...
    {{
      "id": "stock_종목명",
      "label": "종목 분석 - 종목명",
      "narration": "[narration_summary용 요약]",
      "subtitle": "[subtitle_summary용 요약]",
      "narration_summary": "[TTS용 — 주가 정보 + 종목 소개 + 실적/매출 정보 + 시가총액, 시장 5단계로 나눈 내용]",
      "subtitle_summary": "[자막용 — narration_summary용 요약 요약, 시가총액, 등락 자막용 시장, 등락 자막용]",
      "narration_chart": "[TTS용 — 최근 2주 실적 정보 요약 내용 + 실적/분기 매출 정보 + 분기 매출 정보 + 핵심 사업과 시장 성장률, 시장 5단계로 나눈 내용]",
      "subtitle_chart": "[자막용 — narration_chart용 요약 요약, 시가총액, 등락, 자막용 시장, 등락 자막용]",
      "narration_mention": "[TTS용 — 언급 횟수 3회 이하 단독으로 언급]",
      "subtitle_mention": "[자막용 — 언급 횟수 3회 이하 단독으로 언급]",
      "narration_mention_0": "[TTS용 — 언급 횟수 4회 이상 첫 번째 1슬라이드]",
      "subtitle_mention_0": "[자막용 — 언급 횟수 4회 이상 첫 번째 1슬라이드]",
      "narration_mention_1": "[TTS용 — 언급 횟수 4회 이상 두 번째 2슬라이드]",
      "subtitle_mention_1": "[자막용 — 언급 횟수 4회 이상 두 번째 2슬라이드]",
      "summary": "한줄 핵심 요약",
      "price": "000,000",
      "change": "+0.00%",
...
"""
    ...
    # __OPENING__ / __CLOSING__ 플레이스홀더 치환
    def _replace(obj):
