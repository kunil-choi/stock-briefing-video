...
# 변경 5: mention 두 번째 슬라이드 오프닝 멘트 삭제 ("이어서 추가 언급 내용입니다." 제거)
# 변경 6: 경음화 규칙 → 한국어 표준 발음법 기반으로 전면 교체
# 변경 7: 소수점 읽기 "점" → "쩜" 전면 적용
# 변경 8: 숫자+단위 붙여읽기 규칙 추가 (170만원 등)
# 변경 9: 삼성전기 TTS 오독 방지 (띄어쓰기)
...
- 소수점은 반드시 "쩜"으로 읽습니다 ("점" 사용 금지):
  · 12.5% → 십이쩜오퍼센트  /  3.5% → 삼쩜오퍼센트  /  0.8% → 영쩜팔퍼센트
  · X.Y 형태의 모든 소수점은 예외 없이 "쩜"으로 표기
- 숫자+단위는 반드시 붙여서 읽습니다 (중간 공백 금지):
  · 170만원 → 백칠십만원  /  50만원 → 오십만원  /  3,500원 → 삼천오백원
  · "백칠십만...원" 또는 "백칠십만 원" 형태 절대 금지, 반드시 "백칠십만원"으로 붙여 표기
- 영문 약어를 한글 발음으로 읽습니다.
  · SK → 에스케이  /  LG → 엘지  /  KB → 케이비  /  ETF → 이티에프
  · AI → 에이아이  /  HBM → 에이치비엠  /  ESS → 이에스에스
  · KOSPI → 코스피  /  KOSDAQ → 코스닥
- ★ 한국어 표준 발음법 기반 경음화 규칙 (TTS 발음 교정용):
  아래 원칙에 따라 narration 텍스트에 발음대로 표기하되,
  실제 한국어 화자가 자연스럽게 발음하는 방식을 최우선으로 따릅니다.

  [원칙 1] 받침 'ㄱ, ㄷ, ㅂ, ㄹ' 뒤에 오는 'ㄱ, ㄷ, ㅂ, ㅅ, ㅈ'은 경음화:
    · 실적 → 실쩍  /  적자 → 적짜  /  특징 → 특찡
    · 격차 → 격짜  /  국채 → 국째  /  역대 → 역때

  [원칙 2] 한자어에서 'ㄹ' 받침 뒤 'ㄷ, ㅅ, ㅈ' 경음화:
    · 발전 → 발쩐  /  결정 → 결쩡  /  절감 → 절깜

  [원칙 3] 관형사형 어미 '-(으)ㄹ' 뒤 경음화:
    · 할 것 → 할 껏  /  볼 수 → 볼 쑤

  [원칙 4] 합성어·파생어에서 앞 단어 끝 받침 뒤 경음화:
    · 신고가 → 신고까  /  목표가 → 목표까  /  목표주가 → 목표주까
    · 유가 → 유까  /  고유가 → 고유까  /  저유가 → 저유까

  [예외 — 경음화 적용 금지]:
    · 배당금, 삼성전기 등 일반적으로 경음화 없이 읽히는 단어는 원래 표기 유지
    · 고유명사 (기업명, 인명, 브랜드명)는 경음화 적용 금지
    · 확신하기 어려운 경우 경음화를 적용하지 않고 원래 표기를 사용

- ★ 기업명 TTS 오독 방지 규칙:
  · 삼성전기 → "삼성 전기" (띄어쓰기로 TTS 오독 방지. subtitle에는 "삼성전기" 원래 표기 유지)
  · 그 외 기업명도 TTS가 오독할 가능성이 있는 경우 띄어쓰기로 분리
...
- 첫 번째 페이지 (narration_mention 또는 narration_mention_0)만 "각 채널에서 언급한 내용을 보겠습니다."로 시작.
- narration_mention_1, narration_mention_2 등 두 번째 페이지부터는 별도 오프닝 멘트 없이
  바로 첫 번째 언급 내용부터 낭독. "이어서 추가 언급 내용입니다." 문구 절대 사용 금지.
...
- mention 슬라이드 (첫 번째): "각 채널에서 언급한 내용을 보겠습니다."로 시작
  이후 mention 슬라이드: 오프닝 멘트 없이 바로 첫 번째 언급 내용부터 낭독
...
{{
  ...
  "sections": [
    ...
    {{
      ...
      "subtitle_mention_0": "...",
      "narration_mention_1": "[채널명]에서는, [내용] ~라고 전했습니다. ...",
      "subtitle_mention_1": "...",
      "price": "000,000",
      "change": "+0.00%",
